...
echo "RSYNC'ing /spark/spark-ec2 to other cluster nodes..."
...
echo_time_diff "rsync /spark/spark-ec2" "$rsync_start_time" "$rsync_end_time"
...
for module in $MODULES; do
  ...
  cd /spark/spark-ec2  # guard against init.sh changing the cwd
done
...
chmod u+x /spark/spark/conf/spark-env.sh
/spark/spark-ec2/copy-dir /spark/spark/conf
...
for module in $MODULES; do
  ...
  cd /spark/spark-ec2  # guard against setup.sh changing the cwd
done
