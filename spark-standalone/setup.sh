...
BIN_FOLDER="/spark/spark/sbin"

if [[ "0.7.3 0.8.0 0.8.1" =~ $SPARK_VERSION ]]; then
  BIN_FOLDER="/spark/spark/bin"
fi

# Copy the slaves to spark conf
cp /spark/spark-ec2/slaves /spark/spark/conf/
/spark/spark-ec2/copy-dir /spark/spark/conf

# Set cluster-url to standalone master
echo "spark://""`cat /spark/spark-ec2/masters`"":7077" > /spark/spark-ec2/cluster-url
/spark/spark-ec2/copy-dir /spark/spark-ec2
