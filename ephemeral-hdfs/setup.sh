...
EPHEMERAL_HDFS=/spark/ephemeral-hdfs

# Set hdfs url to make it easier
HDFS_URL="hdfs://$PUBLIC_DNS:9000"
echo "export HDFS_URL=$HDFS_URL" >> ~/.bash_profile

pushd /spark/spark-ec2/ephemeral-hdfs > /dev/null
source ./setup-slave.sh

for node in $SLAVES $OTHER_MASTERS; do
  echo $node
  ssh -t -t $SSH_OPTS root@$node "/spark/spark-ec2/ephemeral-hdfs/setup-slave.sh" & sleep 0.3
done
wait

/spark/spark-ec2/copy-dir $EPHEMERAL_HDFS/conf
